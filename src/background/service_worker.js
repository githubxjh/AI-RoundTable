...
async function broadcastMessage(text, targets, attachments = [], options = {}) {
    ...
    const useAdvancedAttachments = shouldUseAdvancedAttachments(options);
    const advancedCleanup = createDeferredDownloadCleanup(chrome, {
        delayMs: ADVANCED_ATTACHMENT_CLEANUP_DELAY_MS
    });
    let advancedStagePromise = null;
    let advancedDownloadIds = [];
    const sentModels = [];
    const degraded = [];
    const skipped = [];
    const failed = [];
    ...
    disabledTargets.forEach((model) => {
        const record = buildAttachmentRecord(model, {
            status: hasAttachments ? ATTACHMENT_STATUS.unsupported : ATTACHMENT_STATUS.none,
    ...
    try {
        for (const [model, tabId] of Object.entries(activeTabs)) {
            ...
            const capability = hasAttachments
                ? getAttachmentCapability(model, normalizedAttachments, { advanced: useAdvancedAttachments })
            ...
            if (hasAttachments && capability.method === ATTACHMENT_METHODS.cdpAdvanced) {
                ...
                try {
                    ...
                    const response = await sendMessageToTab(tabId, {
                        ...
                        mode: 'normal',
                        attachments: [],
                        preuploadedAttachmentCount: normalizedAttachments.length
                    });

                    const record = buildAttachmentRecord(model, {
                        status: response?.status === 'input_simulated'
                            ? ATTACHMENT_STATUS.supported
                            : ATTACHMENT_STATUS.failed,
                        method: ATTACHMENT_METHODS.cdpAdvanced,
                        code: response?.status === 'input_simulated'
            ...
            const method = hasAttachments ? capability.method : ATTACHMENT_METHODS.none;
            const successRecord = buildAttachmentRecord(model, {
                status: hasAttachments ? ATTACHMENT_STATUS.supported : ATTACHMENT_STATUS.none,
                method,
                code: hasAttachments ? capability.code : 'no_attachments',
                reason: hasAttachments ? capability.reason : 'No attachments provided'
            });
            results.push({ model, phase: hasAttachments ? 'attachments' : 'text', response, ...successRecord });

            if (response?.status === 'input_simulated') {
                sentModels.push(model);
                attachmentResults.push(successRecord);
                continue;
            }
            ...
            if (response?.status === 'skipped_unsupported_attachment') {
                const record = buildAttachmentRecord(model, {
                    status: ATTACHMENT_STATUS.unsupported,
                    method: method || ATTACHMENT_METHODS.none,
                    code: String(response?.code || 'attachment_upload_failed'),
                    reason: String(response?.message || 'Attachment upload is unsupported on this model page')
                });
                skipped.push(record);
            ...
            const record = buildAttachmentRecord(model, {
                status: hasAttachments ? ATTACHMENT_STATUS.failed : ATTACHMENT_STATUS.none,
                method,
                code: failure.code,
        ...
        if (advancedDownloadIds.length > 0) {
            advancedCleanup(advancedDownloadIds).catch((error) => {
                console.warn('Advanced attachment cleanup failed:', error);
            });
        }
    }
...
function normalizeBroadcastAttachments(attachments) {
    return normalizeAttachmentPayloads(attachments);
}

function shouldUseAdvancedAttachments(options = {}) {
    if (!isAdvancedAttachmentBuild()) return false;
...
function isAdvancedAttachmentBuild() {
    ...
    return permissions.has('debugger') && permissions.has('downloads');
}

function buildAttachmentRecord(model, input = {}) {
    const status = String(input.status || ATTACHMENT_STATUS.none);
    const method = String(input.method || ATTACHMENT_METHODS.none);
    return {
        model,
        attachmentStatus: status,
        method,
        code: String(input.code || status || 'attachment_status_unknown'),
...
async function prepareAttachmentInputForCdp(tabId, model) {
    const response = await sendMessageToTab(tabId, {
        type: 'PREPARE_ATTACHMENT_INPUT',
        model
    });
    if (response?.status !== 'attachment_input_ready') {
