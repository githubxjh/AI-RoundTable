...
2026-05-30 产品决策：统一附件上传和 Advanced/local 附件线暂停，未来几个月默认不继续投入。当前开发重心是 Lite/public 的低权限、可分享、稳定文本群发和评审流程。本文保留为以后显式恢复附件实验时的收敛规则。
...
- 不把 Advanced 附件调试作为默认下一步；只有用户明确恢复附件实验时才进入下面的附件矩阵。
...
当前附件群发已暂停，不作为 Lite/public 近期目标。若用户未来明确恢复附件实验，不能直接从“全模型群发”开始；按下面顺序升阶，前一层没有证据时不要进入下一层。
