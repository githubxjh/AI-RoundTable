...
async function onBroadcast() {
    ...
    const attachmentMode = getRuntimeAttachmentMode();
    if (attachments.length > 0) {
        const summary = summarizeAttachmentCapabilities(targets, attachments, {
            advanced: attachmentMode === 'advanced'
        });
        if (summary.hasManualRequired) {
            ...
            const confirmed = await showConfirm(
                ...
                {
                    confirmLabel: t('confirmContinue', '继续'),
                    cancelLabel: t('confirmCancel', '取消')
                }
            );
            if (!confirmed) {
                setBroadcastStatus(
                    'warn',
                    t('attachmentBroadcastCancelled', '已取消带附件发送。')
                );
                return;
            }
        }
...
function localizeBroadcastIssueReason(item) {
    ...
    const attachmentStatus = String(item?.attachmentStatus || '').trim();
...
async function serializeFile(file) {
    ...
    return {
        name: file.name,
        mimeType: String(file.type || ATTACHMENT_EXT_TO_MIME[getAttachmentExtension(file.name)] || ''),
        size: file.size,
        base64: dataUrl.split(',')[1] || ''
    };
}

function getRuntimeAttachmentMode() {
    const permissions = new Set(chrome.runtime.getManifest?.().permissions || []);
    return permissions.has('debugger') && permissions.has('downloads') ? 'advanced' : 'lite';
}
...
function isSupportedFile(file) {
    return isAttachmentFileSupported(file);
}
