...
class AdapterBase {
        ...
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            ...
            if (message.type === 'INPUT_PROMPT') {
                ...
                this.handlePrompt({
                    ...
                    preuploadedAttachmentCount: message.preuploadedAttachmentCount
                })
            ...
            if (message.type === 'PREPARE_ATTACHMENT_INPUT') {
                this.prepareAttachmentInput()
                    ...
                    .catch(err => {
                        ...
                        sendResponse({
                            status: 'error',
                            code: code || 'attachment_input_not_found',
                            message: err?.message || 'Attachment input preparation failed'
                        });
                    });
                return true;
            }
    ...
    async handlePrompt(payload = {}) {
        ...
        const preuploadedAttachmentCount = Number(payload?.preuploadedAttachmentCount || 0);

        this._hasAttachments = attachments.length > 0 || preuploadedAttachmentCount > 0;

        if (attachments.length > 0) {
            await this.attachFiles(attachments);
        } else if (preuploadedAttachmentCount > 0) {
            await this.waitForPreuploadedAttachments(preuploadedAttachmentCount);
        }
    ...
    async waitForPreuploadedAttachments(count) {
