    ...
    async prepareAttachmentInput() {
        ...
        if (existing) {
            return {
                ...
                inputVisible: this.isElementVisible(existing),
                multiple: Boolean(existing.multiple)
            };
        }
        ...
        for (const node of candidates) {
            ...
            if (!this.isSendButtonAvailable(target)) continue;
            this.simulateClick(target);
            await this.delay(220);
            const opened = selector ? document.querySelector(selector) : null;
            if (opened) {
                return {
                    status: 'attachment_input_ready',
                    inputSelector: selector,
                    inputVisible: this.isElementVisible(opened),
                    multiple: Boolean(opened.multiple)
                };
            }
        }
        ...
        throw this.createAttachmentError(
            ...
            'DeepSeek attachment input did not open in automated mode'
        );
    }
